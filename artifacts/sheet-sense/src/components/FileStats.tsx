  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 w-full" data-testid="file-stats-grid">
      <StatCard
        ...
        label={t.fileStats.fileName}
        value={file.fileName}
        testId="stat-filename"
      />
      <StatCard
        icon={<Layers className="w-5 h-5 text-primary" />}
        label={t.fileStats.totalSheets}
        value={file.sheetNames.length.toLocaleString()}
        testId="stat-sheets"
      />
      <StatCard
...
function StatCard({
  ...
  return (
    <div
      className="bg-card border border-border rounded-xl p-5 flex items-start gap-4 shadow-sm hover:shadow-md transition-shadow duration-200"
      data-testid={testId}
    >
      <div className="mt-0.5 shrink-0 bg-primary/10 p-2 rounded-lg">{icon}</div>
